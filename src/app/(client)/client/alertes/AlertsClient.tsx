  ...
  React.useEffect(() => {
    const channel = supabase
      ...
      .on(
        ...
        (payload: any) => {
          // Format the DB record to match the UI Incident struct
          const newAlert: Incident = {
            id: payload.new.id.substring(0, 8),
            level: payload.new.severity === 'critical' ? 'CRITIQUE' : payload.new.severity === 'high' ? 'ÉLEVÉ' : 'MODÉRÉ',
            title: `Alerte reçue en temps réel: ${payload.new.message || 'Nouvel incident'}`,
            description: payload.new.ai_analysis?.description || 'Détails non disponibles',
            confidence: payload.new.ai_analysis?.confidence ? `${Math.round(payload.new.ai_analysis.confidence * 100)}% Confiance` : 'En attente',
            time: "À l'instant",
            icon: 'alert-triangle',
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-neutral-200 rounded-2xl p-5 shadow-sm flex items-center justify-between">
          <div className="space-y-1">
            ...
            <div className="text-2xl font-black text-neutral-800">{stats.activeCount}</div>
            <span className="text-[10px] text-red-500 font-bold flex items-center gap-0.5">📈 +{stats.recentCount} depuis 1h</span>
          </div>
        ...
        <div className="bg-white border border-neutral-200 rounded-2xl p-5 shadow-sm">
          ...
          <div className="text-2xl font-black text-neutral-850">{stats.globalRisk}</div>
          <div className="w-full bg-neutral-100 rounded-full h-1.5 mt-2">
            <div className={`h-1.5 rounded-full ${stats.globalRisk === 'Critique' ? 'bg-red-600' : 'bg-[#047857]'}`} style={{ width: stats.globalRisk === 'Critique' ? '90%' : '70%' }} />
          </div>
        </div>
        ...
        <div className="bg-white border border-neutral-200 rounded-2xl p-5 shadow-sm flex items-center justify-between">
          <div className="space-y-1">
            ...
            <div className="text-2xl font-black text-neutral-850">{stats.avgResponseTime}</div>
        ...
        <div className="bg-white border border-neutral-200 rounded-2xl p-5 shadow-sm flex items-center justify-between">
          <div className="space-y-1">
            ...
            <div className="text-2xl font-black text-neutral-850">{stats.impactedSites}</div>
            <span className="text-[10px] text-neutral-400 font-bold">Sur {stats.totalSites} sites actifs</span>
          </div>
          <div className="w-8 h-8 rounded-xl bg-neutral-50 flex items-center justify-center text-neutral-600">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white border border-neutral-200 rounded-2xl p-5 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-black text-neutral-850">Flux des Alertes Actives</h2>
            ...
            <div className="space-y-3.5">
              {incidents.map((incident) => {
                const Icon = getIcon(incident.icon);
                return (
                  <div 
                    key={incident.id} 
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-neutral-200 rounded-2xl p-5 shadow-sm space-y-4">
            ...
            <div className="space-y-4 relative pl-3 border-l-2 border-neutral-100">
              <div className="text-center py-4 text-neutral-400 font-bold">
